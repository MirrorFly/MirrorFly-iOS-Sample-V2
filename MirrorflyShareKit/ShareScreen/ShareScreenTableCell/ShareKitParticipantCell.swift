...
class ShareKitParticipantCell: UITableViewCell {
    ...
    func setTextColorWhileSearch(searchText: String,profileDetail: ProfileDetails) {
        ...
        let name = FlyUtils.getUserName(jid: profileDetail.jid,name: profileDetail.name, nickName: profileDetail.nickName, contactType: profileDetail.contactType)
        if let range = name.range(of: tempSearchText, options: [.caseInsensitive, .diacriticInsensitive]) {
            ...
            nameUILabel?.textColor = ShareKitColor.userNameTextColor
        }
    }
    ...
    func setImage(imageURL: String?, name: String, color: UIColor , recentChat : RecentChat) {
        ...
        weak var imageView = self.contactImageView
    ...
    func setImage(imageURL: String, name: String, color: UIColor , profile : ProfileDetails) {
        ...
        weak var imageView = contactImageView
        imageView?.sd_setImage(with: url, placeholderImage: placeHolder)
    }
    
    func setRecentChatDetails(recentChat: RecentChat,color: UIColor) {
        nameUILabel?.text = FlyUtils.getUserName(jid: recentChat.jid,name: recentChat.profileName, nickName: recentChat.nickName, contactType: (recentChat.isDeletedUser ? .deleted :  recentChat.isItSavedContact ? .live : .unknown))
            ...
            case .audio:
                if let chatMessage = ChatManager.getMessageOfId(messageId: recentChat.lastMessageId) {
                    if chatMessage.mediaChatMessage?.audioType == AudioType.recording {
                        Utility.setIconForAudio(imageView: receiverMessageTypeImageView, chatMessage: chatMessage)
                    } else {
